...
class IRallyMeterApp extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // The distance engine must run app-wide too: it owns tunnel detection, and
    // a tunnel entered while the map or stage timer is showing has to be caught
    // just the same. Watching the delta stream (not just the engine) keeps the
    // increments flowing to the trip/average integrators below.
    ref.watch(distanceEngineProvider);
    ref.watch(distanceDeltaProvider);
    ref.watch(tripProvider);
    // Accumulate average speed app-wide too, so it keeps integrating no matter
